  ...
  useEffect(() => {
    ...
    const handleSend = async () =>{
      const { data, error } = await supabase
        .from('messages')
        // .eq(message_id, "ran")
        .insert([{ from: uuid, to: checkuuid, is_chat: false, username, created_at: new Date().toTimeString() }]);
     if(error){
  ...
  return (
    ...
    {serviceProviderData?.map((provider)=>(
        ...
        <div className="serviceProvider__1">
         ...
         :
         <h1 onClick={handleSend} className='underline text-blue-400 hover:text-blue-600 text-xl md:text-2xl cursor-pointer'>Get Connect</h1>
